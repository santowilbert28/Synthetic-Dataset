...
def setup_camera():
    ...
    # Set render resolution to 4K Ultra HD (3840 x 2160)
...
def setup_lighting_and_environment(hdri_path):
    """Sets up environment lighting using an HDRI optimized for 30m camera height."""
    ...
    # Adjusted background strength for 30m camera height
    background.inputs['Strength'].default_value = random.uniform(0.5, 1.5)

    # Add a Sun lamp optimized for 30m camera height
    sun_height = random.uniform(35, 45)  # Sun above camera
    bpy.ops.object.light_add(type='SUN', location=(random.uniform(-15, 15), random.uniform(-15, 15), sun_height))
    sun_light = bpy.context.object
    sun_light.data.energy = random.uniform(1.5, 4.0)
...
def main():
    ...
    for i in range(NUM_IMAGES):
        ...
        if random_hdri:
            ...
            sun_height = random.uniform(35, 45)
            bpy.ops.object.light_add(type='SUN', location=(random.uniform(-15, 15), random.uniform(-15, 15), sun_height))
            sun_light = bpy.context.object
            sun_light.data.energy = random.uniform(1.5, 4.0)
            sun_light.rotation_euler = (math.radians(random.uniform(30, 70)), math.radians(random.uniform(-45, 45)), random.uniform(0, 2*math.pi))
